...
| 42 | 15:38 CPU search player (64 deals, margin 8, 1 thread/worker) vs plain hybrid, seed 7, paired under the recording load | **+35 vs +162** → −127; four pairs now read +180 / +127 / +11 / −127 (mean +48): the search's edge over the net is unproven |
| 43 | 15:42 parity check: the CPU (eager) and CUDA (graph) evaluators return identical values for identical deals, except the first CUDA call, which was the graph-capture call and returned garbage for one decision (fixed: capture, then reload the state). So the seed-7 CPU result is benchmark noise, not a CPU/GPU difference | head-to-head `pimc` vs `neural_mythosbid` (same bidder and endgame; only the early tricks differ) on seeds 7 / 11 running to measure the search's edge without Mythos's load sensitivity |
| 44 | 15:57 **head-to-head** search hybrid (CPU, 64 deals, margin 8) vs plain hybrid, 512 rounds, same bidder and endgame on both sides | seed 7: 0.53 / **+92**; seed 11: 0.56 / **+88** (nats 36 vs 51) — the search's edge over the net is real and this protocol is far less noisy than paired runs vs Mythos; seeds 13 / 17 and a 32-deal variant queued |
| 45 | 16:00 soft-target distillation trials on the first values checkpoint (2000 rounds, 17k early-trick decisions): warm start, `--min-hand 6`, T = 0.5 / 3 / 5 / 10, lr 1e-4 | agreement with the **search's best card** on held-out decisions is 45–50 % for the untouched net and does not rise with any temperature; at lr 1e-3 × 20 epochs the net memorises the training set (44 → 83 %) while held-out agreement goes 45 → 48 %: **the search's choices are not predictable from the features beyond what the net already knows** |
| 46 | 16:06 search self-consistency: the same 27 early-trick states evaluated twice with independent 64-deal samples | the two argmaxes agree only **67 %** of the time (value correlation 0.84, median best-second gap 7 points): the search's per-decision choice is itself noisy, so ~67 % is the ceiling for any predictor of it, the net's 45–50 % is not far below, and the search's +90/game comes from averaging noise at decision time — which a deterministic policy cannot absorb from its labels |
...
| 55 | 17:18 `v4_regret_soft.pt` vs the current net, head-to-head plain hybrids, seeds 7 / 11 | **+3 / +20** → a tie within noise: a 9 % regret reduction on the search's own values does not show up as strength. Last open avenue: cleaner targets from a 256-deal recording (stopped after ~15 games on the user's decision) |
| 56 | 17:19 **closed** on the user's decision: the 256-deal recording stopped after ~15 games (`training_data_pimc_v2_vals256.npz` keeps the first checkpoint if one was written) | the campaign's outcome table below stands |

## Outcome

**The weights in `neural_best.pt` stay; the public `neural` opponent is now
the net plus net-rollout search** (`PIMCNetPlayer`, `NEURAL_SEARCH=1` by
default, `NEURAL_SEARCH=0` restores the plain net).
...
plateau for seven runs, and the search's gain is decision-time averaging over
sampled deals — its per-position choice is too noisy to serve as a label and
the net already sits near that noise ceiling.
...
- `PIMCNetPlayer` as the public `neural` opponent: 64 deals (128 on CUDA), margin 8, top-8
  ...
  CPU (`NEURAL_PIMC_DEVICE`, `NEURAL_PIMC_THREADS`), CUDA-graph captured on a
  GPU (first-call capture bug fixed).
- The head-to-head benchmark protocol (`--candidate pimc --baseline
  neural_mythosbid`, or a checkpoint via `--model` vs the default net): same
  bidder and endgame on both sides, far less noisy than runs against Mythos.
- Tooling for the next attempt: `values` recording of the search's per-card
  values, `train_neural.py --soft-temp` / `--min-hand` / search-agreement
  metric, `tools/screen_checkpoints.py`, datasets
  `training_data_mythos_v2_50k.npz` (9600 rounds),
  `training_data_pimc_v2.npz` + `_b.npz` (8000 rounds, hard labels),
  `training_data_pimc_v2_vals.npz` (3750 rounds with values).

Untried ideas that could still move the weights: a value head trained on the
search's values (regression instead of choice imitation), features that
expose the sampled-deal statistics the search averages over, or a wider net
trained from scratch on a much larger values recording.
